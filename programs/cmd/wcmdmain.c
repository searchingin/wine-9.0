...
static BOOL has_delimiters(WCHAR *path, const WCHAR *delims)
{
    BOOL ret = FALSE;
    int cc = 0;

    while (path[cc] && !ret) {
        ret = wcschr(delims, path[cc++]) ? TRUE : FALSE;
    }
...
static void update_input_buffer(WCHAR *inputBuffer, const DWORD inputBufferLength, SEARCH_CONTEXT *sc)
{
     ...
     */
    if (sc->have_quotes && !sc->user_specified_quotes && !has_delimiters(&inputBuffer[sc->search_pos], PATH_SEPARATION_DELIMS)) {
     ...
     */
    needQuotes = has_delimiters(sc->fd[sc->current_entry].cFileName, PATH_SEPARATION_DELIMS);
