...
static UINT16 bluez_dbus_mask_from_string_array( DBusMessageIter *arr_iter,
                                                 const struct bluez_object_property_masks *prop_masks, SIZE_T len )
{
    UINT16 mask = 0;

    while (p_dbus_message_iter_get_arg_type( arr_iter ) != DBUS_TYPE_INVALID)
    {
        const char *prop_name;
        SIZE_T i;

        assert( p_dbus_message_iter_get_arg_type( arr_iter ) == DBUS_TYPE_STRING );
        p_dbus_message_iter_get_basic( arr_iter, &prop_name );
        for (i = 0; i < len; i++)
        {
            if (!strcmp( prop_masks[i].prop_name, prop_name ))
...
static DBusHandlerResult bluez_filter( DBusConnection *conn, DBusMessage *msg, void *user_data )
{
             ...
             p_dbus_message_has_signature( msg, DBUS_PROPERTIES_CHANGED_SIGNATURE ))
    {
        ...
        if (!strcmp( iface, BLUEZ_INTERFACE_ADAPTER ))
        {
            ...
            props_changed.invalid_props_mask = bluez_dbus_mask_from_string_array(
                                                  ...
                                                  struct list *gatt_chars_list )
{
    ...
    while((path = bluez_next_dict_entry( &paths_iter, &iface_iter )))
    {
        ...
        while ((iface = bluez_next_dict_entry ( &iface_iter, &prop_iter )))
        {
            ...
            {
                const static struct bluez_object_property_masks flag_masks[] = {
                    { "broadcast", WINEBLUETOOTH_GATT_CHARACTERISTIC_FLAGS_BROADCAST },
                    { "read", WINEBLUETOOTH_GATT_CHARACTERISTIC_FLAGS_READ },
                    { "write", WINEBLUETOOTH_GATT_CHARACTERISTIC_FLAGS_WRITE },
                    { "write-without-response", WINEBLUETOOTH_GATT_CHARACTERISTIC_FLAGS_WRITE_WITHOUT_RESPONSE },
                    { "notify", WINEBLUETOOTH_GATT_CHARACTERISTIC_FLAGS_NOTIFY },
                    { "indicate", WINEBLUETOOTH_GATT_CHARACTERISTIC_FLAGS_INDICATE },
                    { "authenticate-signed-writes", WINEBLUETOOTH_GATT_CHARACTERISTIC_FLAGS_WRITE_SIGNED },
                    { "extended-properties", WINEBLUETOOTH_GATT_CHARACTERISTIC_FLAGS_EXTENDED_PROPS },
                };
                ...
                while ((prop_name = bluez_next_dict_entry( &prop_iter, &variant )))
                {
                    ...
                    {
                        DBusMessageIter flags;
                        p_dbus_message_iter_recurse( &variant, &flags );
                        init_entry->object.characteristic.flags =
                            bluez_dbus_mask_from_string_array( &flags, flag_masks, ARRAY_SIZE( flag_masks ) );
                    }
                             ...
                             && p_dbus_message_iter_get_arg_type( &variant ) == DBUS_TYPE_STRING)
                    {
                        const char *uuid_str;
                        p_dbus_message_iter_get_basic( &variant, &uuid_str );
                        if (!parse_uuid( &init_entry->object.characteristic.uuid, uuid_str ))
                            ERR( "Failed to parse UUID %s for GATT characteristic %s\n", debugstr_a( uuid_str ), path );
                    }
                    else if (!strcmp( prop_name, "Handle" )
                             && p_dbus_message_iter_get_arg_type( &variant ) == DBUS_TYPE_UINT16)
                        p_dbus_message_iter_get_basic( &variant, &init_entry->object.characteristic.handle );
                }
                if (!service_name)
                {
