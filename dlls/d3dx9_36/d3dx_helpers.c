...
HRESULT d3dx_save_pixels_to_memory(struct d3dx_pixels *src_pixels, const struct pixel_format_desc *src_fmt_desc,
        enum d3dx_image_file_format file_format, struct d3dx_buffer *dst_buffer)
{
    ...
    switch (file_format)
    {
        case D3DX_IMAGE_FILE_FORMAT_DDS:
        {
            ...
            hr = dst_buffer->ops->d3dx_buffer_create(dst_buffer, (dst_slice_pitch * src_pixels->size.depth) + header_size);
            if (FAILED(hr))
                return hr;
        ...
        case D3DX_IMAGE_FILE_FORMAT_TGA:
        {
            ...
            hr = dst_buffer->ops->d3dx_buffer_create(dst_buffer, dst_slice_pitch + sizeof(*header));
            if (FAILED(hr))
                return hr;
    ...
    if (src_pixels->size.width != 0 && src_pixels->size.height != 0)
    {
        ...
        if (!dst_buffer->buffer_data)
        {
            ...
            if (FAILED(hr))
                goto exit;
            hr = dst_buffer->ops->d3dx_buffer_create(dst_buffer, buf_size);
            if (FAILED(hr))
            {
    ...
    else if (!dst_buffer->buffer_data)
    {
        ...
        hr = dst_buffer->ops->d3dx_buffer_create(dst_buffer, 64);
        if (FAILED(hr))
            goto exit;
    }
...
exit:
    free(tmp_buf);
    if (FAILED(hr))
        dst_buffer->ops->d3dx_buffer_destroy(dst_buffer);
    return hr;
}
