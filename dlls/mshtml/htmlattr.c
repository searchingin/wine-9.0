...
static HRESULT HTMLDOMAttribute_clone(HTMLDOMNode *iface, nsIDOMNode *nsnode, HTMLDOMNode **ret)
{
    ...
    hres = HTMLDOMAttribute_Create(NULL, This->elem, This->dispid, This->node.doc, nsattr, &new_attr);
    nsIDOMAttr_Release(nsattr);
    if(FAILED(hres))
        return hres;
...
HRESULT HTMLDOMAttribute_Create(const WCHAR *name, HTMLElement *elem, DISPID dispid, HTMLDocumentNode *doc,
                                nsIDOMAttr *nsattr, HTMLDOMAttribute **attr)
{
    ...
    if(nsattr) {
        ...
        HTMLDOMNode_Init(doc, &ret->node, (nsIDOMNode*)nsattr, &Attr_dispex);
        *attr = ret;
        return S_OK;
    }
