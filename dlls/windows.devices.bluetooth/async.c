...
struct async_result_BluetoothAdapter
{
...
static inline struct async_result_BluetoothAdapter *impl_from_IAsyncOperation_BluetoothAdapter( IAsyncOperation_BluetoothAdapter *iface )
{
    return CONTAINING_RECORD( iface, struct async_result_BluetoothAdapter, IAsyncOperation_BluetoothAdapter_iface );
}

static HRESULT WINAPI async_result_QueryInterface( IAsyncOperation_BluetoothAdapter *iface, REFIID iid, void **out )
{
    struct async_result_BluetoothAdapter *impl = impl_from_IAsyncOperation_BluetoothAdapter( iface );
    ...
    {
        IInspectable_AddRef( (*out = &impl->IAsyncOperation_BluetoothAdapter_iface) );
        return S_OK;
    }
...
static ULONG WINAPI async_result_AddRef( IAsyncOperation_BluetoothAdapter *iface )
{
    struct async_result_BluetoothAdapter *impl = impl_from_IAsyncOperation_BluetoothAdapter( iface );
    ULONG ref = InterlockedIncrement( &impl->ref );
    TRACE( "iface %p, ref %lu.\n", iface, ref );
...
static ULONG WINAPI async_result_Release( IAsyncOperation_BluetoothAdapter *iface )
{
    struct async_result_BluetoothAdapter *impl = impl_from_IAsyncOperation_BluetoothAdapter( iface );
    ULONG ref = InterlockedDecrement( &impl->ref );
    TRACE( "iface %p, ref %lu.\n", iface, ref );
...
static HRESULT WINAPI async_result_GetIids( IAsyncOperation_BluetoothAdapter *iface, ULONG *iid_count, IID **iids )
{
    FIXME( "iface %p, iid_count %p, iids %p stub!\n", iface, iid_count, iids );
    return E_NOTIMPL;
}

static HRESULT WINAPI async_result_GetRuntimeClassName( IAsyncOperation_BluetoothAdapter *iface, HSTRING *class_name )
{
    static const WCHAR class[] = L"Windows.Foundation.IAsyncOperation`1<Windows.Devices.Bluetooth.BluetoothAdapter>";
    TRACE( "iface %p, class_name %p\n", iface, class_name );
    return WindowsCreateString( class, ARRAY_SIZE( class ) - 1, class_name );
}

static HRESULT WINAPI async_result_GetTrustLevel( IAsyncOperation_BluetoothAdapter *iface, TrustLevel *trust_level )
{
    FIXME( "iface %p, trust_level %p stub!\n", iface, trust_level );
    return E_NOTIMPL;
}

static HRESULT WINAPI async_result_put_Completed( IAsyncOperation_BluetoothAdapter *iface, IAsyncOperationCompletedHandler_BluetoothAdapter *handler )
{
    struct async_result_BluetoothAdapter *impl = impl_from_IAsyncOperation_BluetoothAdapter( iface );
    TRACE( "iface %p, handler %p.\n", iface, handler );
...
static HRESULT WINAPI async_result_get_Completed( IAsyncOperation_BluetoothAdapter *iface, IAsyncOperationCompletedHandler_BluetoothAdapter **handler )
{
    struct async_result_BluetoothAdapter *impl = impl_from_IAsyncOperation_BluetoothAdapter( iface );
    TRACE( "iface %p, handler %p.\n", iface, handler );
...
static HRESULT WINAPI async_result_GetResults( IAsyncOperation_BluetoothAdapter *iface, IBluetoothAdapter **results )
{
    struct async_result_BluetoothAdapter *impl = impl_from_IAsyncOperation_BluetoothAdapter( iface );
    ...
    if (SUCCEEDED( hr ))
        hr = IUnknown_QueryInterface( result.punkVal, &IID_IBluetoothAdapter, (void **)results );
    PropVariantClear( &result );
    return hr;
}

static const struct IAsyncOperation_BluetoothAdapterVtbl async_result_BluetoothAdapter_vtbl = {
    /* IUnknown methods */
    ...
    /* IAsyncOperation<BluetoothAdapter> */
...
{
    struct async_result_BluetoothAdapter *impl;
    HRESULT hr;

    *out = NULL;
    if (!(impl = calloc( 1, sizeof(*impl) ))) return E_OUTOFMEMORY;
    impl->IAsyncOperation_BluetoothAdapter_iface.lpVtbl = &async_result_BluetoothAdapter_vtbl;
    impl->ref = 1;

    if (FAILED(hr = async_info_create( invoker, param, callback, (IInspectable *)&impl->IAsyncOperation_BluetoothAdapter_iface, &impl->IWineAsyncInfoImpl_inner )) ||
